...
//            captureBuilder.set(LENS_OPTICAL_STABILIZATION_MODE, LENS_OPTICAL_STABILIZATION_MODE_OFF);//Fix ois bugs for preview and burst
    ...
    public void gains() {
        ...
        if (checkdouble(rggbChannelVector.getRed()) && checkdouble(rggbChannelVector.getGreenEven()) && checkdouble(rggbChannelVector.getGreenOdd()) && checkdouble(rggbChannelVector.getBlue())) {
            BiConsumer<String, Integer> setField = (name, index) -> {
                try {
                    Field field = rggbChannelVector.getClass().getDeclaredField(name);
                    field.setAccessible(true);
                    field.set(rggbChannelVector, 1f / WB[index].floatValue());
                } catch (NoSuchFieldException | IllegalAccessException e) {
                    e.printStackTrace();
                }
            };
            setField.accept("mRed", 0);
            setField.accept("mGreenEven", 1);
            setField.accept("mGreenOdd", 1);
            setField.accept("mBlue", 2);
